...
export default function Create() {
  ...
  const handleSubmit = async () => {
    // CRITICAL CHECK: ensure token is present before attempting to submit
    if (!token) {
      Alert.alert("Unauthorized", "You must be logged in to create a book.");
      router.replace("/(auth)");
      return;
    }
    ...
    try {
      ...
      // --- START: Robust Error Handling Fix ---
      let data;
      ...
      try {
        ...
        // If parsing fails, it's non-JSON (HTML/Text error)
        if (!response.ok) {
          console.error("Non-JSON Server Error Response:", responseText);
          throw new Error(
            `Server Error (${response.status}). The server returned an unexpected format (expected JSON).`
          );
        }
        // If the response status was OK but parsing failed, something is wrong with the successful JSON format.
        throw new Error("Failed to parse successful response as JSON.");
      }

      if (!response.ok) {
        // This handles API errors that correctly returned a JSON body (e.g., 400 or 401)
        throw new Error(
